...
const search = useSearch({ strict: false })
  ...
  const tripId = search.trip ? Number(search.trip) : null  // fetch uchun trip id
  const expandedOrderId = search.order ? Number(search.order) : null  // buyurtmani kengaytirish uchun

  // Agar tripId bo'lmasa, return qilish yoki xato ko'rsatish
  if (!tripId) {
    return <div>Trip tanlanmagan</div>  // yoki null
  }
  ...
  const { data, isLoading } = useGet<ListResponse<TripOrdersRow>>(
    ...
    {
      params: {
        id: tripId,  // endi search.trip dan
        page,
      },
    }
  )
  ...
  const toggleExpand = (orderId: number) => {
    ...
    navigate({
      search: ((prev: Record<string, unknown>) => ({
        ...prev,
        order: isOpen ? undefined : String(orderId),  // expanded uchun order param
      })) as any,
    })
  }
